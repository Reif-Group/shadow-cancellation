...
function approx_cat_amp(initX, N, stopTime, figNo)
    ...
    for i = 1:N-1
        r{2*N + i} = addreaction(model, strcat('Gx', int2str(i), ...
            '2 -> x', int2str(i+1)));
    end
    r{3*N} = addreaction(model, strcat('Gx', int2str(N), '2 -> x1'));

    % Reactions in the shadow circuit
    for i = 1: N-1
        r{3*N + 2*i - 1} = addreaction(model, strcat('s', int2str(i), ...
            ' + Gs', int2str(i), '1 -> Is', int2str(i)));
        r{3*N + 2*i} = addreaction(model, strcat('Is', int2str(i), ...
            ' + Gs', int2str(i), '2 -> s', int2str(i), ' + s', int2str(i+1)));
    end
    r{3*N + 2*N - 1} = addreaction(model, strcat('s', int2str(N), ' + Gs', ...
        int2str(N), '1 -> Is', int2str(N)));
    r{3*N + 2*N} = addreaction(model, strcat('Is', int2str(N), ' + Gs', ...
        int2str(N), '2 -> s', int2str(N), ' + s1'));
    
    % Leak reactions in the shadow circuit
    for i = 1: N-1
        r{5*N + i} = addreaction(model, strcat('Gs', int2str(i), ...
            '2 -> s', int2str(i+1))); % changed, check.
    end
    r{6*N} = addreaction(model, strcat('Gs', int2str(N), '2 -> s1'));

    % Annihilation reactions
    for i = 1: N
        r{6*N + i} = addreaction(model, strcat('x', int2str(i), ...
            ' + s', int2str(i), ' -> W'));
    end
    ...
    for i = 1:2:2*N-1
        ...
        p{i+1} = addparameter(k{i+1}, strcat('c', int2str(i+1)), 'Value', q2);
    end
    % Rate params for the leaks
    for i = 2*N+1: 3*N
        p{i} = addparameter(k{i}, strcat('c', int2str(i)), ...
            'Value', leak_rate*q2);
    end
    ...
    for i = 3*N + 1: 2: 5*N - 1
        istr = int2str(i);
        p{i} = addparameter(k{i}, strcat('c', istr), 'Value', q1);
        p{i+1} = addparameter(k{i+1}, strcat('c', int2str(i+1)), 'Value', q2);
    end
    % Rate for the shadow leaks
    for i = 5*N + 1: 6*N
        istr = int2str(i);
        p{i} = addparameter(k{i}, strcat('c', istr), 'Value', leak_rate*q2);
    end

    % Rate for annihilation reactions
    for i = 6*N + 1: 7*N
        istr = int2str(i);
        p{i} = addparameter(k{i}, strcat('c', istr), 'Value', q2);
    end
...
%         r{i}.Reactants(1).InitialAmount = 0.0;    
    ...
    % Set initial concentration for gates 
    for i = 1: 2*N
