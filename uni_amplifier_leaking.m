...
function approx_cat_amp(initX, N, stopTime, figNo)
    ...
    for i = 1: N
        r{2*N + i} = addreaction(model, strcat('Gx', int2str(i), '2 -> x', int2str(i)));
    end
    ...
    for i = 1:N
        p{2*N + i} = addparameter(k{2*N + i}, strcat('c', ...
            int2str(2*N + i)), 'Value', leak_rate*q2);
    end
...
%     set(gca, 'LineWidth', 2.0); legend('auto. X', 'cat. X', 'approx. X');
    set(gca, 'LineWidth', 2.0); legend('leaky.noShadow.X');
end